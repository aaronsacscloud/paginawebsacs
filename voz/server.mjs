...
//
//   wss://…/voz/ws        Twilio ConversationRelay abre uno por llamada
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && ruta === '/salud') {
    return json(res, 200, { ok: true, vivas: sesiones.size, atendidas, modelo: MODELO_VOZ, cerebro: hayCerebro(), crm: hayCrm(), segundos: Math.round((Date.now() - arranque) / 1000) });
  }
...
server.on('upgrade', (req, socket, head) => {
  ...
  if (ruta !== '/voz/ws' && ruta !== '/ws') { socket.destroy(); return; }
  wss.handleUpgrade(req, socket, head, (ws) => wss.emit('connection', ws, req));
});

wss.on('connection', (ws) => {
  atendidas++;
  const s = new Sesion(ws, { alCerrar: (ses) => { if (ses.item && sesiones.get(ses.item) === ses) sesiones.delete(ses.item); } });
  ws.on('message', async (raw) => {
    let msg; try { msg = JSON.parse(raw.toString()); } catch { return; }
    try {
      await s.recibir(msg);
      if (msg.type === 'setup' && s.item) sesiones.set(s.item, s);
    } catch (e) { log(`[${s.item}] error atendiendo ${msg.type}:`, e.message); }
  });
