-- 15-sep-2026 · Novias · cadencia de ESPAÑA (region 'espana').
--
-- Por qué: el dueño pidió «adelante con todo España» tras Latinoamérica, con la
-- misma estrategia que México: mismos 8 correos por ruta (días 1,4,6,10,14,19,
-- 25,33), mismos 3 WhatsApp (días 2,16,36), mismas imágenes y el mismo orden
-- de ideas. Es la cadencia Latam (2026-09-14-abm-novias-latam-cadencia.sql)
-- pasada al español de España, no una copia de la de México.
--
-- Lo que cambia y por qué:
-- · Vocabulario del ramo en España: «punto de venta» → «TPV», «sucursales» →
--   «tiendas», «abonos» → «pagos a cuenta», «anticipo» → «señal», «vitrina» →
--   «escaparate», «traslados» → «traspasos», «vendedora» → «dependienta»,
--   «tienda en línea» → «tienda online», «le acomoda» → «le viene bien»,
--   «Buen día» → «Buenos días». El caso real se cuenta en euros («unos 55.000
--   euros», con punto de miles como se escribe allá).
-- · Sin quince años: en España no existe esa fiesta; el guion de correo nunca
--   la menciona ({{xv}} solo vive en las consultas del barrido).
-- · LSSI art. 21 y RGPD (interés legítimo B2B): el primer correo ya dice quién
--   escribe y de dónde salió el dato («lo buscamos nosotros, en lo que su
--   negocio publica en Google Maps y en su web») y remite a la baja del pie;
--   el pie del pipeline pone identificación, dirección, motivo, aviso de
--   privacidad y baja en un clic (RFC 8058). La revisión con abogado sigue
--   siendo decisión del dueño antes del primer envío (abm-paises.ts, `legal`).
-- · WhatsApp: plantillas `abm_novias_es_*` en idioma es_ES con {{nombre}},
--   {{ciudad}} y {{pais}}. NO están registradas en Meta; se registran solo
--   con el OK del dueño. Hasta entonces el cron las deja en la fila.
-- · Un solo goteo «Novias · España» en `pausado` y las cuentas en `en_pausa`:
--   no sale nada hasta que el dueño revise el reporte. La migración de
--   encendido es otra.
begin;
-- Sin índices únicos en plantillas/pasos/goteo: el candado contra correrla dos veces es este.
do $$ begin if exists (select 1 from abm_plantillas where giro='novias' and region='espana') then raise exception 'la cadencia de España de novias ya existe'; end if; end $$;

-- ── A. Cadencias ─────────────────────────────────────────────────────────────
insert into abm_cadencias (id, nombre, giro, ruta, region, descripcion, activa, creada_por) values
  ('a1c0de11-0000-4000-8000-0000000e5001', 'Novias · demo · España', 'novias', 'demo', 'espana',
   'Casas de novia y fiesta de España. 8 correos + 3 WhatsApp en español de España (TPV, tiendas, señal, escaparate); {{landing}} lleva a la página de España.', true, 'sistema'),
  ('a1c0de11-0000-4000-8000-0000000e5002', 'Novias · diagnóstico · España', 'novias', 'diagnostico', 'espana',
   'Casas de novia con 5+ tiendas en España. Misma estructura que la de México, en español de España.', true, 'sistema')
...
('novias','email','demo','espana','presentacion',0,'por qué le llega este correo',
E'[[si persona]]Hola {{persona}}.\n[[/si]]Le escribo de Sacs, un software de stock y TPV para negocios de moda, con clientes en México y Latinoamérica. Antes que nada, por qué le llega esto: estuvimos levantando el mapa de las casas de novia de {{pais}} y {{nombre}}[[si ciudad]], en {{ciudad}},[[/si]] apareció ahí. Nadie nos pasó su correo ni usted se registró en ningún lado — lo buscamos nosotros, en lo que su negocio publica en Google Maps y en su web. Si no quiere que le escriba más, el enlace de baja está al pie y con eso basta.\nY no le escribo en automático. Esto es lo que vimos de ustedes:[[si senal]]\n· {{senal}}.[[/si]][[si sucursales]]\n· {{sucursales}} tiendas.[[/si]][[si plataforma]]\n· Su tienda online está en {{plataforma}}.[[/si]]\nPor eso pensamos que le podemos servir. Hay mucho TPV genérico y todos hacen lo mismo: cobran y descuentan del inventario. En una casa de novias eso no alcanza, porque la venta no termina en la caja: termina en una fecha.\nTenemos una suite hecha para casas de novias y fiesta. Lo que resuelve:\n· Reserva con fecha del evento, señal y pagos a cuenta, con el saldo al día. Se acabó el recibo en una servilleta.\n· La muestra marcada aparte de los pedidos, para que no se venda la única talla 6 que tenía en tienda.\n· Taller con órdenes de servicio y fechas: qué vestido entra primero y desde cuándo está ahí.\n· Pruebas y entregas con fecha, no de memoria. Nadie vuelve a preguntar si era el 25 o el 29.\n· El pedido al proveedor con su tiempo de entrega, para saber si ese modelo en esa talla llega antes de la boda.\n· Un catálogo que sirve igual en tienda, en WhatsApp y en redes, con el mismo inventario detrás.\nY esto es solo una parte: atrás hay stock por tienda, traspasos entre tiendas, cierre de caja y comisión de la dependienta. Se lo muestro en treinta minutos por videollamada, en su horario, y usted juzga.\nSi prefiere preguntarme por WhatsApp antes de agendar nada: https://wa.me/525593027234\n¿Se lo muestro, o le envío primero cómo se ve por dentro la ficha de una novia?',
'texto','citar lo investigado, explicar por qué el genérico no alcanza y dar las dos ligas','novias-1-presentacion.jpg','[]',true),

('novias','email','demo','espana','novias demo · correo 1',1,'una duda sobre sus reservas',
...
('novias','email','demo','espana','novias demo · correo 2',2,'la talla del muestrario',
E'En novias el inventario no es cuántos vestidos tienen colgados. Es qué modelo, en qué talla y para qué fecha.\n\nUn modelo vive en ocho o diez tallas, y el muestrario casi nunca es la talla de la novia. La pieza que ella se probó se queda en el escaparate y la suya se pide al proveedor con meses de anticipación.\n\nEntre el día que se prueba y el día de la boda pasan pruebas, ajustes, una señal, un pago final y un taller. Si una de esas fechas se atrasa, se atrasa todo lo demás.\n\n¿Cuántos vestidos tienen reservados al mismo tiempo en temporada alta?',
'texto','enseñar el dolor mecánico con números del oficio','novias-3-talla.jpg','[]',true),

('novias','email','demo','espana','novias demo · correo 3',3,'una ficha por novia',
E'Le cuento cómo lo resolvemos, para que juzgue si le sirve.\n\nCada venta deja de ser una línea y se vuelve una ficha con fecha de boda: modelo, talla pedida, qué día es la prueba, qué día lo recibe el taller y qué día se entrega. El sistema cuenta hacia atrás desde la boda, no hacia adelante desde hoy.\n\nEncima va el dinero: señal, pagos a cuenta, saldo y quién autorizó el descuento. Y el pedido especial al proveedor queda amarrado a esa novia, así que cuando llega, llega con nombre y apellido, no a la bodega.\n\n¿Cuántos vestidos entregan en una temporada buena?',
...
E'Le dejo algo que puede hacer esta semana sin comprarme nada.\n\nSaque las ventas del último año y arme dos columnas por modelo: en qué tallas se pidió y cuántos días pasaron entre la reserva y la boda.\n\nLa primera columna le dice qué muestrario debería tener colgado. Muchas casas de novia llenan el escaparate con la talla que se ve bonita en el maniquí y venden otra, y esa diferencia se paga en ajustes de taller.\n\nLa segunda le dice su verdadero tiempo de anticipación. Si su promedio son cuatro meses y su proveedor tarda tres, no tiene margen: cualquier retraso se lo come el taller o la novia.\n\nCon eso ya sabe qué pedir de muestrario y hasta cuándo puede aceptar dos bodas la misma semana.\n\n¿Tiene registradas las fechas de boda del año pasado?',
'texto','dar algo útil aunque no compren','novias-6-tallas.jpg','[]',true),

('novias','email','demo','espana','novias demo · correo 6',6,'lo que aparece en 50 modelos',
E'Un ejemplo de lo que aparece cuando uno mira el inventario en serio.\n\nEn un cliente nuestro, una cadena de moda, revisamos solo 50 modelos y encontramos el equivalente a unos 55.000 euros mal repartidos entre el centro de distribución y las tiendas. Mercancía ya comprada y pagada, solo que no estaba donde la gente la pedía.\n\nEn novias el mismo problema tiene otra cara: muestrarios repetidos de modelos que ya nadie pide, tallas que se maltrataron de tanto probarse y siguen contando como inventario bueno, y señales de vestidos que llevan meses sin llegar.\n\nSi quiere, en treinta minutos por videollamada le muestro el sistema cargado con sus propios modelos y usted juzga si le sirve.\n\n¿Le viene bien esta semana o la próxima?',
'texto','contar el caso real y proponer la demo','novias-7-modelos.jpg','[]',true),

('novias','email','demo','espana','novias demo · correo 7',7,'lo dejo aquí',
E'[[si persona]]{{persona}}, [[/si]]no le quiero seguir llenando el correo, así que lo dejo aquí.\n\nLe escribí porque las casas de novia son de las que peor la pasan con los sistemas de tienda normales: ninguno entiende que una venta empieza hoy y se entrega dentro de seis meses, que hay una señal de por medio y que la pieza pasa por taller antes de salir. Casi todos las tratan como si vendieran camisetas.\n\nSi algún día se les junta una temporada, abren otra tienda o solo quieren ver cómo se vería su catálogo por dentro, me escribe y lo vemos en treinta minutos, en su horario. No le voy a volver a insistir.\n\n¿Le escribo otra vez el próximo año o mejor lo saco de la lista?',
'texto','cerrar con dignidad','novias-8-cierre.jpg','[]',true),

-- ── C. Plantillas de correo · ruta diagnóstico ───────────────────────────────
('novias','email','diagnostico','espana','presentacion',0,'por qué le llega este correo',
E'[[si persona]]Hola {{persona}}.\n[[/si]]Le escribo de Sacs, un software de stock y TPV para negocios de moda, con clientes en México y Latinoamérica. Antes que nada, por qué le llega esto: estuvimos levantando el mapa de las casas de novia de {{pais}} y {{nombre}}[[si ciudad]], en {{ciudad}},[[/si]] apareció ahí. Nadie nos pasó su correo ni usted se registró en ningún lado — lo buscamos nosotros, en lo que su negocio publica en Google Maps y en su web. Si no quiere que le escriba más, el enlace de baja está al pie y con eso basta.\nY no le escribo en automático. Esto es lo que vimos de ustedes:[[si senal]]\n· {{senal}}.[[/si]][[si sucursales]]\n· {{sucursales}} tiendas, que es justo donde empieza el problema: el vestido que una clienta pidió en una casi siempre está colgado en otra.[[/si]][[si plataforma]]\n· Su tienda online está en {{plataforma}}.[[/si]]\nPor eso pensamos que le podemos servir. Hay mucho software genérico y todos hacen lo mismo: cobran y descuentan del inventario. En una casa de novias eso no alcanza, porque la venta no termina en la caja: termina en una fecha.\nTenemos una suite hecha para casas de novias y fiesta: reserva con fecha del evento y pagos a cuenta, la muestra marcada aparte de los pedidos, taller con órdenes de servicio, pruebas y entregas con fecha, y el pedido al proveedor con su tiempo de entrega. Y esto es solo una parte: atrás hay stock por tienda, traspasos y cierre de caja.\nPero antes de mostrarle nada, le ofrezco algo más útil: un diagnóstico gratis de su inventario. Con su información le decimos en quince minutos cuánto dinero tiene parado en modelos que no se mueven, qué tallas sí le venden y cuánto se le va en vestidos que estaban en la tienda equivocada.\nNo es una demo disfrazada: son sus números y se los entregamos aunque no nos compren.\nSi prefiere preguntarme por WhatsApp antes de agendar nada: https://wa.me/525593027234\n¿Le sacamos el diagnóstico con sus números?',
'texto','citar lo investigado y ofrecer el diagnóstico gratis','novias-1-presentacion.jpg','[]',true),

('novias','email','diagnostico','espana','novias diagnostico · correo 1',1,'quince minutos con sus números',
E'[[si persona]]Hola {{persona}}.\n[[/si]]Vi {{nombre}}[[si ciudad]] en {{ciudad}}[[/si]][[si sucursales]] y que tienen {{sucursales}} tiendas[[/si]]. Con varias tiendas de novia, el vestido que una clienta pidió en una casi siempre está colgado en otra.\n\nHacemos inventario para negocios de moda. Ofrecemos un diagnóstico gratis: con su información, en quince minutos les decimos cuánto dinero tienen parado y cuánto se les va por faltantes.\n\n¿Le interesa que lo hagamos con sus números?',
'texto','abrir con lo investigado y ofrecer el diagnóstico','novias-2-apartados.jpg','[]',true),

('novias','email','diagnostico','espana','novias diagnostico · correo 2',2,'dinero parado entre tiendas',
E'Dinero parado es mercancía que ya pagó y que está donde nadie la pide.\n\nEn una tienda sola se nota: usted ve el vestido colgado desde marzo. Con varias tiendas deja de notarse, porque cada encargado ve solo su tienda y todos creen que lo suyo está bien.\n\nY se multiplica por las tallas. Un modelo de novia vive en ocho o diez tallas. Si en la tienda grande sobran las chicas y en la otra faltan, las dos pierden venta al mismo tiempo y en la oficina central aparece como que hay existencia.\n\n[[si sucursales]]Con {{sucursales}} tiendas eso ya no se ve a simple vista.\n\n[[/si]]¿Cada cuánto revisan qué se mueve entre tiendas?',
'texto','el dolor de varias tiendas','novias-3-talla.jpg','[]',true),

('novias','email','diagnostico','espana','novias diagnostico · correo 3',3,'50 modelos, 55.000 euros',
E'Le paso el caso que hace que la gente nos conteste.\n\nEn un cliente nuestro, una cadena de moda, revisamos solo 50 modelos. Encontramos el equivalente a unos 55.000 euros mal repartidos entre el centro de distribución y las tiendas: mercancía ya comprada y pagada que no estaba donde se vendía.\n\nNo fue robo ni mal cierre de mes. Fue reparto. Cada tienda pidió lo que creía y nadie miró el conjunto.\n\nEn novias duele más, porque el vestido que no está el día que la clienta lo pide no se vende después: se va con la competencia y no vuelve.\n\n¿Quiere que hagamos ese mismo ejercicio con sus modelos?',
'texto','contar el caso real','novias-4-ficha.jpg','[]',true),

('novias','email','diagnostico','espana','novias diagnostico · correo 4',4,'sobre pasarnos su información',
...
('novias','email','diagnostico','espana','novias diagnostico · correo 5',5,'cómo medirlo usted mismo',
...
-- ── D. WhatsApp para Meta (idioma es_ES): texto fijo, {{nombre}} {{ciudad}} {{pais}} ──
('novias','whatsapp','ambas','espana','abre',1,null,
E'Buenos días. Le escribo de Sacs — software de stock y TPV para negocios de moda, con clientes en México y Latinoamérica.\n\nEstamos armando el mapa de las mejores casas de novia de {{pais}} y {{nombre}}, en {{ciudad}}, apareció en la lista. Los encontramos en Google Maps; nadie nos pasó su contacto. Si no quiere más mensajes, con responder «no» basta.\n\nLe escribo porque lo nuestro no es un TPV genérico: tenemos una versión hecha para casas de novia. Reservas con la fecha de la boda y sus pagos a cuenta al día, el muestrario marcado aparte de lo que sí se vende, y las pruebas, el taller y el pedido al proveedor con fecha.\n\nEste mes estamos dando demos gratis por videollamada, 30 minutos, en su horario y sin compromiso. ¿Le muestro cómo se vería con sus modelos?',
'texto','WhatsApp · abre. Plantilla de Meta (es_ES): texto fijo con {{nombre}}, {{ciudad}} y {{pais}}. Un día después del primer correo.',null,'[]',true),
('novias','whatsapp','ambas','espana','sigue',2,null,
E'Le escribo una vez más y ya no insisto.\n\nLo que más nos dicen las casas de novia es que todo cuelga de una fecha: el vestido que se pide al proveedor, las pruebas, la señal y el pago final. Si una se atrasa, se atrasan todas — y eso hoy casi siempre vive en un cuaderno o en un grupo de WhatsApp.\n\nEso es justo lo que resuelve la versión para novias: cada novia con su fecha, sus pagos a cuenta y sus pruebas en un solo lugar, y el sistema avisando antes, no cuando ya pasó.\n\nLa demo por videollamada sigue en pie: 30 minutos, gratis, en su horario y sin compromiso. ¿Le viene bien esta semana o la próxima?',
'texto','WhatsApp · sigue. Plantilla de Meta (es_ES) sin huecos. A media cadencia (día 16).',null,'[]',true),
('novias','whatsapp','ambas','espana','cierra',3,null,
E'Con este cierro el tema — no quiero ser el que insiste.\n\nSi en alguna temporada les pesa llevar las reservas, las pruebas y los pagos a cuenta en un cuaderno, aquí seguimos y con gusto les muestro cómo lo resuelve el sistema. Este número queda abierto.\n\nY si prefiere verlo sin hablar con nadie primero, puede agendar la demo cuando quiera con el botón de abajo.\n\nGracias por el tiempo y mucho éxito con la temporada.',
'texto','WhatsApp · cierra. Plantilla de Meta (es_ES) sin huecos, con botón a la agenda. Después del último correo (día 36).',null,'[]',true);
...
join abm_plantillas p on p.giro = 'novias' and p.canal = 'email' and p.region = 'espana' and p.ruta = c.ruta and p.orden = d.pl_orden and p.activa
where c.region = 'espana' and c.giro = 'novias';
...
-- ── F. Un goteo, PAUSADO. Diez al día como en México. ───────────────────────
insert into abm_goteo (cadencia_id, nombre, cuentas_dia, filtro, con_ia, estado, creado_por, nota)
values ('a1c0de11-0000-4000-8000-0000000e5001', 'Novias · España', 10, '{"pais":"es"}'::jsonb, true, 'pausado',
  '60be8bd8-995a-45ca-926f-1bcb159d3c1e',
  'Casas de novia de España (ruta demo). Correo + WhatsApp a quien publicó su wa.me. Pausado hasta que el dueño revise el reporte y la parte legal (LSSI/RGPD) (15-sep-2026).'),
  ('a1c0de11-0000-4000-8000-0000000e5002', 'Novias · diagnóstico · España', 10, '{"pais":"es"}'::jsonb, true, 'pausado',
  '60be8bd8-995a-45ca-926f-1bcb159d3c1e', 'Casas de novia con 5+ tiendas en España. Pausado hasta el OK del dueño.');

select (select count(*) from abm_cadencias where region='espana') cadencias,
       (select count(*) from abm_plantillas where region='espana') plantillas,
       (select count(*) from abm_pasos p join abm_cadencias c on c.id=p.cadencia_id where c.region='espana') pasos,
       (select count(*) from abm_goteo where estado='pausado' and nombre like 'Novias · %España%') goteos_pausados;
commit;
