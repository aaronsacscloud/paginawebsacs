...
bk as (
  select contact_id, count(*) citas_total, count(*) filter (where estado = 'asistio') citas_asistio,
         count(*) filter (where estado = 'no_asistio') citas_no_asistio, count(*) filter (where estado in ('agendada','confirmada','reagendada')) citas_vigentes,
...
select k.id contact_id, k.nombre, k.apellido, k.fuente, k.utm_source, k.utm_campaign, k.campana, k.created_at, k.lifecycle_stage, k.estatus_lead,
       ...
       coalesce(bk.citas_total, 0) citas_total, coalesce(bk.citas_asistio, 0) citas_asistio, coalesce(bk.citas_no_asistio, 0) citas_no_asistio, coalesce(bk.citas_vigentes, 0) citas_vigentes, bk.primera_cita_at,
