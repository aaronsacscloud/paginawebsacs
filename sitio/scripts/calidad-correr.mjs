...
const { analizarCompetencia, juzgar, generarPortada } = await import('../src/lib/demanda/calidad.ts');
...
for (let ronda = 0; ronda <= 2; ronda++) {
  ...
  if (ronda === 2) {
    ...
    console.log('  → atascada tras 2 reescrituras; la ve el dueño con el veredicto al lado');
    break;
  }
  // reescribir
  const { data: fresh } = await supabase.from('de_contenido').select('brief').eq('id', c.id).single();
  brief = { ...fresh.brief, correcciones: v.fallos, reescrituras: ronda + 1, veredicto_anterior: v.puntajes };
