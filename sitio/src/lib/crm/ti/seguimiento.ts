...
export async function inferirCriterio(original: string, final: string, mensajeLead?: string | null): Promise<string | null> {
  ...
  const r = await anthropic.messages.create({ proposito: 'lib/crm/ti/seguimiento.ts:309', model: MODELS.sonnet, max_tokens: 120, messages: [{ role: 'user', content: `Un consultor corrigió la respuesta de un agente de WhatsApp (ventas de software para tiendas de moda). Deduce la REGLA general detrás del cambio, en una sola línea imperativa y concreta que sirva para casos parecidos (no describas el cambio, formula la regla). Si el cambio es solo de estilo trivial, dilo en la regla igual («corta a dos frases»).\nLead: «${String(mensajeLead || '').slice(0, 240)}»\nAgente: «${String(original).slice(0, 500)}»\nConsultor: «${String(final).slice(0, 500)}»\nResponde SOLO la regla, sin comillas ni prefijo.` }] });
