...
/** Una foto → lo que se ve, en 2-3 líneas. Sonnet: barato y ve bien. */
export async function describirFoto(url: string, mime?: string | null): Promise<{ texto: string | null; costo: number }> {
  ...
  const r: any = await anthropic.messages.create({
    ...
    messages: [{ role: 'user', content: [{ type: 'image', source: { type: 'base64', media_type: img.mime as 'image/jpeg' | 'image/png' | 'image/gif' | 'image/webp', data: img.b64 } }, { type: 'text', text: PROMPT }] }],
  });
...
export async function describirFotosDe(msjs: any[], max = 3): Promise<{ descritas: number; costo: number }> {
  ...
  const pend = msjs.filter(m => m.direccion === 'entrante' && m.tipo === 'image' && m.media_url && m.id && (m.transcript == null || m.transcript === '')).slice(-max);
  for (const m of pend) {
    try {
...
export async function describirFotosPendientes(opts: { dias?: number; max?: number } = {}): Promise<any> {
  ...
  const { data: fotos } = await supabase.from('wa_mensajes').select('id, media_url, mime, created_at')
    .eq('tipo', 'image').eq('direccion', 'entrante').is('transcript', null).is('borrado_at', null)
