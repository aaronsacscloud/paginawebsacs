...
- ⚠️ «NO RECUERDA» NO ES «NO LE INTERESA» (medido en 68 llamadas reales: el caso más común, 29 %, es «¿de dónde?» / «yo no me registré» / «no sé»). Si el contacto no reconoció a Sacs o su registro, pero NO dijo claramente que no le interesa, el resultado es "contesto" (o "volver_llamar" si pidió tiempo), la etapa es null —NUNCA descalificado— y en "envios" va «la información de Sacs» (usa su conocimiento_id) para que vea quiénes somos. Lo mismo si dijo «déjame checarlo», «le regreso la llamada» o «mándame un mensajito»: eso es interés que falta concretar, no un no.
- Un «gracias» o un «te agradezco» seco sin más contexto, después de no reconocer a Sacs, TAMPOCO es no_interesa: es que no sabe quiénes somos. Mismo trato que el punto anterior.
