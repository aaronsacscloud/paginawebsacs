...
export const GET: APIRoute = async ({ url, request }) => {
  ...
  /* ── Un grupo unificado es UN cobro, y por lo tanto UN aviso ──────────
     Al unificar fechas, varias licencias pasan a cobrarse juntas el mismo día.
     Este cron recorre suscripción por suscripción, así que le mandaba al
     cliente un correo POR LICENCIA: Boom Fitness recibió dos avisos de
     vencimiento el mismo 2 de septiembre, uno por $21,420 y otro por $7,140,
     por una sola deuda. Dos correos por lo mismo se leen como un error de
     cobranza, que es lo último que uno quiere mandar cuando está cobrando.

     Se elige un VOCERO por grupo —la licencia más cara, que es la que se nombra
     en la conversación— y solo ese avisa, con el total del grupo. Los demás
     siguen haciendo sus cambios de estado, que sí son de cada una. Se agrupa
     por grupo Y por fecha: si a una se le movió el día, ya no están unificadas
     de hecho y cada una avisa por su cuenta. */
  const grupoTotal: Record<string, number> = {};
  const grupoN: Record<string, number> = {};
  const vocero: Record<string, string> = {};
  for (const s of (subs || []) as any[]) {
    if (!s.grupo_cobro || !s.proxima_factura) continue;
    const k = `${s.grupo_cobro}|${String(s.proxima_factura).slice(0, 10)}`;
    grupoTotal[k] = (grupoTotal[k] || 0) + (Number(s.monto_proximo ?? s.precio) || 0);
    grupoN[k] = (grupoN[k] || 0) + 1;
    const actual = vocero[k] ? (subs as any[]).find(x => x.id === vocero[k]) : null;
    if (!actual || (Number(s.precio) || 0) > (Number(actual.precio) || 0)) vocero[k] = s.id;
  }
  ...
  for (const s of (subs || []) as any[]) {
    try {
      ...
      const clave = s.grupo_cobro ? `${s.grupo_cobro}|${String(s.proxima_factura).slice(0, 10)}` : null;
      const enGrupo = !!(clave && grupoN[clave] > 1);
      // Callado: hace sus cambios de estado pero no le escribe al cliente.
      const calla = enGrupo && vocero[clave!] !== s.id;
      const monto = enGrupo ? Math.round(grupoTotal[clave!]) : (Number(s.monto_proximo ?? s.precio) || 0);
      const queEs = enGrupo ? `${grupoN[clave!]} licencias unificadas` : s.nombre_plan;

      // ── recordatorios previos ── (no a quien ya pidió cancelar al vencer)
      const hitos = s.ciclo === 'anual' ? [30, 15, 7] : [3];
      if (dias >= 0 && hitos.includes(dias) && !s.cancela_al_vencer && !calla) {
        ...
        if (!(await yaAvisado(s.id, hito))) {
          if (contacto.email) {
            await notify({
              ...
              data: { empresa, contacto: contacto.nombre, plan: queEs, ciclo: s.ciclo, fecha: fmtD(s.proxima_factura), monto },
            }).catch(() => null);
          }
      ...
      if (dias < 0) {
        ...
        if (vencidaDias >= 1 && !calla && !(await yaAvisado(s.id, 'dunning_email_' + s.proxima_factura))) {
          if (contacto.email) {
            await notify({
              channel: 'email', to: contacto.email, template: 'arr_payment_overdue',
              data: { empresa, contacto: contacto.nombre, plan: queEs, fecha: fmtD(s.proxima_factura), dias: vencidaDias, monto },
            }).catch(() => null);
            out.dunning_email++;
          }
        ...
        if (vencidaDias >= 3 && !calla && await permitido('cobranza') && !(await yaAvisado(s.id, 'dunning_wa_' + s.proxima_factura))) {
          if (contacto.whatsapp) {
            await sendWhatsApp(contacto.whatsapp,
              `Hola ${contacto.nombre || ''} 👋 Te escribimos de SACS: el pago de tu suscripción ${queEs} venció el ${fmtD(s.proxima_factura)} ($${monto.toLocaleString('es-MX')} MXN). ¿Te ayudamos a regularizarlo? Si ya pagaste, mándanos tu comprobante por aquí. 🙌`, 'Sistema', 'cliente').catch(() => null);
            out.dunning_wa++;
          }
          await marcarAviso(s, 'dunning_wa_' + s.proxima_factura, `📱 Dunning día 3 (WhatsApp): ${empresa} · $${monto.toLocaleString('es-MX')}`);
        }
        if (vencidaDias >= 7 && !calla && !(await yaAvisado(s.id, 'dunning_tarea_' + s.proxima_factura))) {
