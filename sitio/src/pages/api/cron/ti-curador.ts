...
async function pares() {
  ...
  for (const p of lista || []) {
    try {
      const r = await anthropic.messages.create({ proposito: 'pages/api/cron/ti-curador.ts:27', model: MODELS.opus, max_tokens: 300, messages: [{ role: 'user', content: `Eres el curador del agente SDR de Sacscloud (retail de moda). Mismo turno, dos respuestas al lead. Lead dijo: «${String((p.salida as any)?.ultimo_mensaje || '').slice(0, 300)}». Estado del guion: ${(p.salida as any)?.estado || '?'}.\n\nA (agente): ${String(p.mensaje).slice(0, 600)}\n\nB (humano): ${String(p.humano_respuesta).slice(0, 600)}\n\n¿Cuál enseña mejor cómo contestar (corta, una pregunta, sin admiraciones, fondo correcto, siguiente paso natural)? Responde SOLO JSON: {"mejor":"A|B|empate","razon":"1 línea"}` }] });
