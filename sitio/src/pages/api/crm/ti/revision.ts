...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (b.accion === 'rampa') { const cfg: any = await leerConfig(); const r = { ...(cfg.rampa_revision || { aceptadas: 0 }), automatico: !!b.automatico }; await parcharConfig({ rampa_revision: r }); return json({ ok: true, rampa: r }); }
