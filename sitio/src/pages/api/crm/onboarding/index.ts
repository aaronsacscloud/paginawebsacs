...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (b.accion === 'abrir') {
    ...
    const { data: co } = await supabase.from('companies').select('id, nombre, sacs_account, uso_sacs, owner_id').eq('id', companyId).maybeSingle();
    ...
    const { error } = await supabase.from('onboarding_casos').insert({
      company_id: companyId, consultor_id: co.owner_id || null, uso_al_abrir: co.uso_sacs || null,
    });
