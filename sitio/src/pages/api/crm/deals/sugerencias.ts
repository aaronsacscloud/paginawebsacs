...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (b?.generar) {
    ...
    const [renov, senales] = await Promise.all([generarRenovaciones(dias, stage), generarDeSenales(stage)]);
    return json({ ok: true, renovaciones: renov, upsells: senales, dias });
  }
