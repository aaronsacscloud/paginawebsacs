...
export default function PipelineKanban({ stages, items, getId, getStage, onMove, renderCard, emptyLabel = 'Sin etapa' }: {
  stages: KStage[];
  items: any[];
  getId: (t: any) => string;
  ...
  const Col = ({ st, its, droppable }: { st: { key: string; label: string; color?: string }; its: any[]; droppable: boolean }) => (
    <div
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 6px 10px', fontWeight: 700, fontSize: '0.8rem', color: droppable ? '#333' : '#999' }}>
