  ...
  return (
    <div className={mobile ? 'wa-hilo-m' : undefined} style={{ position: 'relative', flex: 1, minWidth: mobile ? 0 : L.hiloMin, display: 'flex', flexDirection: 'column', minHeight: 0, borderLeft: mobile ? 'none' : `1px solid ${C.g200}`, background: mobile ? '#fff' : C.g50, height: mobile ? 'calc(100dvh - 64px)' : undefined }}>
      ...
      {conv.alerta && (() => {
        ...
        /* Los límites de Meta ya no llegan hasta aquí: se dejan de guardar
           como alerta de la conversación en `espejo.ts`, que es donde nacían.
           El parche por texto que estuvo aquí un día perseguía uno de los cinco
           códigos de esa familia; el resto habría vuelto a salir. */
      ...
      <Composer key={`${conv.id || conv.email_only_id}-${composerN}`} ventana={hilo.ventana} api={api} telefono={conv.telefono} equipo={equipo} movil={mobile}
        ...
        canales={{ ...hilo.canales, wa_id: conv.id, linea: conv.phone_number_id || null }}
