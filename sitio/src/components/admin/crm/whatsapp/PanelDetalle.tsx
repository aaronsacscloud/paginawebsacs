  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ width: L.railito, flexShrink: 0, borderLeft: `1px solid ${C.g100}`, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 6, gap: 4 }}>
        {/* ══ OCULTAR, ARRIBA DEL TODO (20-sep-2026) ═══════════════════════
            Pedido del dueño: «el botón de ocultar ponlo donde está el más,
            para que tenga más sentido que sólo sea el icono del colapsable, y
            el de + ponlo abajo de ése».

            Tiene razón en las dos cosas. El riel es la columna de los iconos
            que ABREN Y CIERRAN cosas: ahí es donde la mano va a buscar el que
            cierra la ficha, no entre las pestañas de contenido. Y en el riel
            basta el icono —la chevron ya dice «recoger»— mientras que en la
            barra de pestañas hacía falta la palabra para no perderse.

            El + de Acciones baja un renglón: es la única acción del riel que
            CREA algo, y empezar por ella la ponía por delante de lo que se usa
            cada minuto. */}
        <button onClick={() => document.dispatchEvent(new CustomEvent('wa-plegar-ficha'))}
          ...
          style={{ width: 32, height: 32, borderRadius: 8, border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'none', color: C.g400 }}>
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" aria-hidden><path d="M9 6l6 6-6 6" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
        {TABS.map(t => {
  ...
  return (
    <div style={{ border: `1px solid ${C.g100}`, borderLeft: `3px solid ${C.emerald500}`, borderRadius: 10, marginBottom: 6, overflow: 'hidden' }}>
      ...
      {l.minuta_pdf_cliente_url && (
        <a href={l.minuta_pdf_cliente_url} target="_blank" rel="noreferrer"
          style={{ display: 'flex', alignItems: 'center', gap: 7, borderTop: `1px solid ${C.g100}`, padding: '7px 10px', fontSize: 11, fontWeight: 700, color: C.emerald700, textDecoration: 'none', background: '#F7FCFA' }}>
          <span style={{ fontSize: 12 }}>📤</span>
          <span style={{ flex: 1, minWidth: 0 }}>Versión para el cliente</span>
          {l.minuta_envio_estado === 'enviada' && <span title="Ya se le mandó" style={{ fontSize: 9, fontWeight: 700, background: C.emerald50, color: C.emerald700, borderRadius: 999, padding: '1px 6px' }}>enviada</span>}
          {l.minuta_envio_estado === 'pendiente_ventana' && <span title="Sale en cuanto responda" style={{ fontSize: 9, fontWeight: 700, background: C.ambar100, color: C.ambar700, borderRadius: 999, padding: '1px 6px' }}>por enviar</span>}
        </a>
      )}
      {l.minuta_pdf_url && (
        <a href={l.minuta_pdf_url} target="_blank" rel="noreferrer"
          ...
          <span style={{ fontSize: 12 }}>📄</span>
          <span style={{ flex: 1, minWidth: 0 }}>Minuta interna <span style={{ fontWeight: 500, color: C.g400 }}>· con todo</span></span>
        </a>
      )}
