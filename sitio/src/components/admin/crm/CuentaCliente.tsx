...
import { faltanFiscales } from '../../../lib/crm/fiscal.ts';
...
export default function CuentaCliente({ companyId, alCambiar }: { companyId: string; alCambiar?: () => void }) {
  ...
  /* `fisc === null` es «todavía no se sabe», no «están completos»: darlos por
     buenos mientras cargan cerraría el recuadro medio segundo antes de
     descubrir que faltan. */
  const faltan = fisc !== null && faltanFiscales(fisc);
  ...
  if (st.cuenta && !pruebaViva && !faltan && !listo) return null;
  ...
  return (
    <div style={{ border: '1px solid #e6ddfa', background: '#fbfaff', borderRadius: 12, padding: '13px 16px', margin: '10px 0' }}>
      ...
      {faltan && (
        <div style={{ borderTop: '1px solid #ece7f8', marginTop: 12, paddingTop: 12, maxWidth: 420 }}>
          <DatosFiscales companyId={companyId} fisc={fisc}
            onGuardado={d => { setFisc({ ...(fisc || {}), ...d }); alCambiar?.(); }} />
        </div>
      )}
    </div>
  );
