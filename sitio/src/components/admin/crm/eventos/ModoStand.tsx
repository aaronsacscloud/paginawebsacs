...
export default function ModoStand({ edicion, registros, meta, onCerrar, onNuevo }: {
  ...
  const deHoy = registros.filter(r => String(r.capturado_at).slice(0, 10) === hoy).length;

  useEffect(() => {
    const sync = async () => { const k = await sincronizarCola(edicion.id); setPendientes(pendientesDe(edicion.id)); if (k) { setAviso({ t: `Se mandaron ${k} registros que esperaban red.`, tono: 'bien' }); onNuevo(); } };
    sync();
