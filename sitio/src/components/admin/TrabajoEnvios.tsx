...
export default function TrabajoEnvios() {
  ...
  const cargar = () => fetch('/api/crm/ti/envios').then(r => r.json()).then(j => { if (j.error) { setBanner({ texto: j.error, tipo: 'err' }); return; } setPend(j.pendientes || []); setRec(j.recientes || []); setCfg(j.config || null); setApr(j.aprendizaje || null); }).catch(() => setBanner({ texto: 'No se pudieron cargar los envíos', tipo: 'err' }));
