...
## 2026-09-07 · Plantillas, nombres y propuestas viejas
- **«Hola WhatsApp».** El nombre del contacto era un placeholder («WhatsApp 0021») y las plantillas metían la primera palabra tal cual. Regla: el {{1}} se arma AL SALIR con `saludoParaPlantilla` (nombre real o «buenas tardes»), nunca al nacer la sugerencia. Y el perfil de WhatsApp sustituye al placeholder cuando es nombre de persona.
- **En 298 conversaciones el nombre de perfil llegó vacío.** Kapso no lo manda en `payload.contact.name` como asumíamos. Se amplió la ruta y se anota un diagnóstico por hora con las llaves del payload para corregirla con datos, no adivinando.
- **Modificar no cambiaba lo que salía.** Si la plantilla ya venía colgada, {{2}} conservaba el texto original. Ahora {{2}} se recalcula al salir si el consultor editó.
- **Respaldo a los 10 minutos era tarde y ciego.** Meta rechaza marketing (131049) en segundos. El espejo del envío del agente ahora lleva `respaldo_plan`, el webhook de fallo manda la utility al instante y deja una línea de sistema en el hilo con la hora. El reloj de 10 min queda de red y no duplica.
- **La marca del agente avanzaba aunque fallara.** Sin créditos, 6 leads que escribieron («Sí, cuéntame», «Estoy de viaje», «no tengo solvencia») quedaron huérfanos para siempre. `ti_huerfanos(7)` los devuelve a la fila en cada tick, sin importar la marca.
- **Las propuestas viejas se retiran por evento, no por barrido.** El lead escribe → los toques preparados se reemplazan; el consultor contesta → las sugerencias se califican como «humano» y los pendientes se vetan, al instante.

## 2026-09-07 · Rafael y Yalile: «parecemos desesperados»
- **Dos ciclos a la vez = doble respuesta.** El cron cada 2 min y el observador que dispara el panel al abrirse redactaban los dos el mismo turno (10 s de diferencia). El índice de «un pendiente por lead» no lo evita porque el primero ya salió cuando llega el segundo. Candado `ti_lock('respuesta:<cid>', 180 s)` antes de redactar.
